...
uintptr_t GetUObjectFromPool(DeadByDaylight& dbd,uintptr_t base,int index) 
{
	...
	uintptr_t objArray = dbd.Read<uintptr_t>(gObjects);
...
static unsigned int lastKillerIndex = 0;
...
{
	{
		...
		while (dbd.CheckGameAlive()) {
			...
			if (GWorld) {
				...
				killer = FindSingleActorOfType(dbd, persistent, killerKlass, &lastKillerIndex);
								...
								if (pawn) {
									...
									if (skillCheck) {
										...
										if (isDisplayed) {
											...
											if (IsKillerCarryingMe(dbd, pawn)) {
